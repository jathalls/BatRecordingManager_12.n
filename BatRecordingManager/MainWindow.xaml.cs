        ...
        public MainWindow()
        {
            try
            {
                ...
                try
                {
                    ...
                    var dc = DBAccess.OpenDatabase(App.dbFileLocation, App.dbFileName);
                    // calls SetDatabase which calls InitializeDatabase
                    if (dc == null)
                    {
                        MessageBox.Show("Unable to create or initialize the database.  Try re-installing SqlLocal2019.msi", "FATAL ERROR");
                        this.Close();
                    }
                }
                catch (Exception ex)

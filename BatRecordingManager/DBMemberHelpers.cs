...
namespace BatRecordingManager
{
    ...
    public static class DbMemberHelpers
    {
        ...
        public static void ImportWavFile(this RecordingSession session, string file)
        {
            ...
            var listOfSegmentAndBatList = new ObservableCollection<SegmentAndBatList>();
    ...
    public partial class Call
    {
        ...
        public ReferenceCall ToRefCall()
        {
            ...
            Bat bat = null;
            if (this.BatCalls != null && this.BatCalls.Count > 0)
            {
        ...
        public DateTime StartDateTime
        {
            get
            {
                ...
                else
                {
                    ...
                    if (Recording != null)
                    {
                        if (Recording.RecordingDate != null && Recording.RecordingDate.Value.Second == 0 &&
                            Recording.RecordingDate.Value.Millisecond == 0)
                        {
                            Recording.RepairTimes();
                        }
                        if (Recording.RecordingStartTime == null)
                        {
                            Recording.RecordingStartTime = (Recording.RecordingDate ?? new DateTime()).TimeOfDay;
                        }
    ...
    public partial class Recording
    {
        ...
        /// <summary>
        /// Bug fix at 16Jan2022.  Recording start and end times as stored in RecordingStartTime and RecordingDate are only set to the nearest
        /// minute and do not contain seconds or milliseconds.  If detected this function is called and takes measures to ensure that start and end
        /// times and fully qualified dateTime are as precise as the data allows, using a combination of metadata, GPS and filename.  The rocording in the databse is updated
        /// and the current instance is modified as well.  If best efforts determine that the seconds field should in fact be zero, or is
        /// indeterminable, then the millisecnds fied is set to 1 to prevent this function being called in future for the same recording.
        /// This function is called primarily from Tools.FormattedSegmentLine and should therefore be applied to any recording which has its details
        /// displayed.
        /// </summary>
        public void RepairTimes()
        {
            Tools.GetFileDatesAndTimes(this.GetFileName(), out string wavFile, out DateTime start, out DateTime end);
            this.RecordingDate = start;
            this.RecordingStartTime = start.TimeOfDay;
            this.RecordingEndTime = end.TimeOfDay;
            if (this.RecordingStartTime.Value.Seconds == 0 && this.RecordingStartTime.Value.Milliseconds == 0)
            {
                this.RecordingDate += new TimeSpan(0, 0, 0, 0, 1);
                RecordingStartTime = RecordingDate.Value.TimeOfDay;
            }
            DBAccess.UpdateRecording(this);

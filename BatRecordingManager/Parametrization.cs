...
namespace BatRecordingManager
{
    ...
    internal class Parametrization
    {
        ...
        public Parametrization(List<float> alldata, int sampleRate, int AnalysisMode = 5)
        {
            ...
            {
                Tools.InfoLog($"Exception in Parametrization: {ex.Message} ");
            }
        }
        ...
        public List<Peak> AllPeaks { get; set; } = new List<Peak>();
        ...
        internal void AnalysePulse(System.Windows.Point pe)
        {
            try
            {
                if (unfilteredPeaks.Count > 0)
                {
                    ...
                    if (!displayAnalysis(displayableData))
                    {
                        Debug.WriteLine("Unable to display DisplayableData from AnalysePulse(point)");
            ...
            {
                Tools.InfoLog($"AnalysePule(point) : {ex.Message}");
            }
        }
        ...
        internal void AnalysePulse(List<float> sectionData, int startSample)
        {
            try
            {
                ...
                if (!displayAnalysis(displayableData))
                {
                    Debug.WriteLine("Unable to display DisplayableData from AnalysePulse(sectiondata,startSample)");
                    //var analysisWindow = new CallAnalysisWindow();
                    //analysisWindow.WindowState = System.Windows.WindowState.Maximized;
                    //analysisWindow.Show();
                    //analysisWindow.AnalysisChartGrid.saveClicked += AnalysisChartGrid_saveClicked;
            ...
            {
                Tools.InfoLog($"AnalysePulse(data,sample): {e.Message}");
            }
        }
        ...
        /// <summary>
        /// Checks to see if BatCallAnalysisA.exe existis on this computer, and if so builds
        /// a string containing all details of all analysed calls and places it on the clipboard.
        /// Then runs, or gives the focus to BatCallAnalysisA, which will check the clipboard for
        /// text starting [BRM] when started.
        /// </summary>
        /// <param name="displayableData"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        private bool displayAnalysis(List<CallData> displayableData)
        {
            try
            {
                string BCAPath = getBatCallAnalyserPath();
                Debug.WriteLine($"Starting ~{BCAPath}~");

                if (!string.IsNullOrWhiteSpace(BCAPath))
                {
                    string clipBoardText = BuildClipBoardText(displayableData);
                    if (!string.IsNullOrWhiteSpace(clipBoardText) && clipBoardText.StartsWith("[BRM"))
                    {
                        Debug.WriteLine($"\n Put on the clipboard:-\n{clipBoardText}\n");
                        System.Windows.Clipboard.SetText(clipBoardText);
                        var alreadyRunning = Process.GetProcessesByName("BatCallAnalyserA");
                        if (alreadyRunning != null && alreadyRunning.Length > 0)
                        {
                            foreach (var runner in alreadyRunning)
                            {
                                Debug.WriteLine($"Killing {runner.MainWindowTitle}");
                                //runner.Kill();
                                Thread.Sleep(1000);
                            }
                        }
                        ...
                        try
                        {
                            ...
                            process.StartInfo.FileName = BCAPath;
                            process.StartInfo.WindowStyle = ProcessWindowStyle.Maximized;
                            process.Start();
                        }
                        catch (ObjectDisposedException ode)
                        {
                            Debug.WriteLine($"ODE object already disposed: {ode.Message}");
                            Tools.InfoLog($"ODE object already disposed: {ode.Message}");
                        }
                        catch (InvalidOperationException ioe)
                        {
                            Debug.WriteLine($"IOE no file name specified:- {ioe.Message}");
                            Tools.InfoLog($"IOE no file name specified:- {ioe.Message}");
                        }
                        catch (Win32Exception w32e)
                        {
                            Debug.WriteLine($"W32E error opening file:- {w32e.Message}");
                            Tools.InfoLog($"W32E error opening file:- {w32e.Message}");
                        }

                        catch (PlatformNotSupportedException pnse)
                        {
                            Debug.WriteLine($"PNSE OS not supported: {pnse.Message}");
                            Tools.InfoLog($"PNSE OS not supported: {pnse.Message}");
                        }
                        //Debug.WriteLine(process.StartInfo);
                        ...
                        //}
                        //Thread.Sleep(10000);
                        //SetForegroundWindow((Int32)process.MainWindowHandle);
                        var p = Process.GetProcessesByName("BatCallAnalyserA");
                        if (p != null && p.Length > 0)
                        {
                            Debug.WriteLine($"BCA running {p[0].MainWindowTitle}/{process.MainWindowTitle}, {process.StartTime}");
                            //Tools.InfoLog($"BCA running {p[0].MainWindowTitle}/{process.MainWindowTitle}, {process.StartTime}");
                        }
                        else
                        {
                            Debug.WriteLine($"hasExited={process.HasExited},\n {process.StartTime}-{process.ExitTime} code={process.ExitCode}");
                            //Tools.InfoLog($"hasExited={process.HasExited},\n {process.StartTime}-{process.ExitTime} code={process.ExitCode}");
                        }
                        return (true);
                    }
                }
            ...
            {
                Tools.InfoLog($"displayAnalysis: {ex.Message}");
            }
            return (false);
        }
        ...
        private void Process_Exited(object sender, EventArgs e)
        {
            try
            {
                ...
                Debug.WriteLine($"BCA Exited at {p.ExitTime} with code {p.ExitCode}");
            }catch(Exception ex)
            {
                Tools.InfoLog($"Process Exited: {ex.Message}");
            }
        }

        /// <summary>
        /// Given a valid List of CallData creates a clipboard string commencing with [BRM and having a 
        /// separate line for each call with named call aparameters
        /// </summary>
        /// <param name="displayableData"></param>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        private string BuildClipBoardText(List<CallData> displayableData)
        {
            string result = "";
            try
            {
                
                if (displayableData != null && displayableData.Count > 0)
                {
                    result = "[BRM v12.1]\n";
                    foreach (CallData callData in displayableData)
                    {
                        result += callData.ToString() + "\n";
                    }
                }
            }catch(Exception ex)
            {
                Tools.InfoLog($"BuildClipboardText: {ex.Message}");
            }
        ...
        /// <summary>
        /// Looks for a copy of BatCallAnalyserA.exe, first in the current folder, then in ProgramFiles, and
        /// then in programFilesx86, then c:echolocation
        /// </summary>
        /// <returns></returns>
        /// <exception cref="NotImplementedException"></exception>
        private string getBatCallAnalyserPath()
        {
            try
            {
                string path = Path.Combine(".", @"BatCallAnalyserA\BatCallAnalyserA.exe");
                if (File.Exists(path)) return (path);
                path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), @"BatCallAnalyserA\BatCallAnalyserA.exe");
                if (File.Exists(path)) return (path);
                path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), @"Echolocation\BatCallAnalyserA\BatCallAnalyserA.exe");
                if (File.Exists(path)) return (path);
                path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), @"BatCallAnalyserA\BatCallAnalyserA.exe");
                if (File.Exists(path)) return (path);
                path = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86), @"Echolocation\BatCallAnalyserA\BatCallAnalyserA.exe");
                if (File.Exists(path)) return (path);
                path = Path.Combine(@"C:\Echolocation", @"BatCallAnalyserA\BatCallAnalyserA.exe");
                if (File.Exists(path)) return (path);
            }catch(Exception ex)
            {
                Tools.InfoLog($"getBatCallAnalyserPath: {ex.Message}");
            }
            return (null);
        }

        internal bool CalculateParameters(List<Spectrum> spectra, int FFTSize, int FFTOverlapSamples)
        {
            try
            {
            ...
            {
                Tools.InfoLog($"CalculateParameters: {ex.Message}");
            }
            return (false);
        }
        ...
        private ReferenceCall getCallFromAllData()
        {
            ...
            {
                Tools.InfoLog($"getCallFromAllData: {ex.Message}");
            }
            return (call);
        }
        ...
        private bool getCalls()
        {
            try
            {
                ...
                if (envelopeData == null) return false;
                ...
                //Tools.InfoLog($"envelope of {envelopeData.Count}");
                ...
                //Tools.InfoLog($"got {AllPeaks.Count} peaks");
                ...
                AllPeaks = (from pk in AllPeaks
                            ...
                            select pk)?.ToList();
                if (AllPeaks == null) return false;
                //Tools.InfoLog($"selected {AllPeaks.Count} peaks");
                GetFrequencyParams();
                ...
                //Tools.InfoLog($"Analysing {AllPeaks.Count} pulses");
#if DEBUG
                ...
                ReferenceCall call = getCallFromAllData(); // uses data in AllPeaks and in AllFrequencyData
                string err=call?.setToSettings();
                if (!string.IsNullOrWhiteSpace(err))
                {
                    Tools.InfoLog($"setToSettings Error:{err}");
                }
                //Tools.InfoLog($"set {call?.fStart_Mean}-{call?.fEnd_Mean}");
                List<CallData> displayableData = getDisplayableData(); // get the details of each analysed call
                //Tools.InfoLog($"displaying {displayableData.Count} calls");
                if (displayableData == null) return false;
                if (!displayAnalysis(displayableData))
                {
                    Debug.WriteLine("Unable to display DisplayableData from getCalls()");
                    return (false);
                    //Tools.InfoLog("Unable to display DisplayableData");
                    //var analysisWindow = new CallAnalysisWindow();
                    //analysisWindow.WindowState = System.Windows.WindowState.Maximized;
                    //analysisWindow.Show();
            ...
            {
                Tools.InfoLog($"getCalls: {ex.Message}");
            }
            return (true);
        ...
        private List<CallData> getDisplayableData()
        {
            ...
            {
                Tools.InfoLog($"getDisplayableData: {ex.Message}");
            }
            return (data);
        }
        ...
        private FrequencyDataSet getFrequencydata(Peak peak, bool isFirstPeak = false, string fname = @"C:\BRMTestData\freq.csv")
        {
            ...
            {
                Tools.InfoLog($"getFrequencyData: {ex.Message}");
            }
            return (result);
        }

...
namespace BatRecordingManager
{
    ...
    public class BatStats
    {
        ...
        public BatStats(string batName, LabelledSegment segment)
        {
            //TimeSpan duration = endTime - startTime;
            TimeSpan duration = segment.EndOffset - segment.StartOffset;
            if (duration.Ticks < 0) duration = new TimeSpan((long)Math.Abs(duration.Ticks));
            maxDuration = TimeSpan.MinValue;
            minDuration = TimeSpan.MaxValue;
            meanDuration = new TimeSpan();
            totalDuration = new TimeSpan();
            ...
            batCommonName = batName;
            batAutoID = "";
            this.unsure = segment.isConfidenceLow;
        ...
        /// <summary>
        /// the time and date of the start of the first pass of this bat
        /// </summary>
        public DateTime firstOccurrence { get; set; }

        /// <summary>
        /// the time and date of the end of the last pass of this bat
        /// </summary>
        ...
        /// <summary>
        /// Adds details from the parameter segment to the batstats table for that bat.
        /// </summary>
        /// <param name="segment"></param>
        public void Add(LabelledSegment segment)
        {
            
            Add(segment.EndOffset - segment.StartOffset, segment.AutoID, segment.isConfidenceLow);
            if (firstOccurrence.Ticks == 0) firstOccurrence = segment.StartDateTime;
            DateTime start = segment.StartDateTime;
            DateTime end = segment.StartDateTime + (segment.Duration() ?? new TimeSpan());
            if (start < this.firstOccurrence) this.firstOccurrence = start;
            if(end>this.lastOccurrence) this.lastOccurrence = end;
        }
        ...
        public void Add(BatStats newData)
        {
            ...
            if (newData != null && newData.count > 0)
            {
                ...
                meanDuration = new TimeSpan(totalDuration.Ticks / count);
                if (this.firstOccurrence.Ticks == 0) firstOccurrence = newData.firstOccurrence;
                if (newData.firstOccurrence.Ticks > 0 && newData.firstOccurrence < this.firstOccurrence) this.firstOccurrence = newData.firstOccurrence;
                if (this.lastOccurrence.Ticks == 0) this.lastOccurrence = newData.lastOccurrence;
                if (newData.lastOccurrence.Ticks > 0 && newData.lastOccurrence > this.lastOccurrence) this.lastOccurrence = newData.lastOccurrence;
            }
        }

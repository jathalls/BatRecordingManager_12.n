...
namespace UniversalToolkit
{
    public class NMEA2OSG
    {
        ...
        public bool ParseNMEA(string nlat, string nlon, double height,bool OSTN=false)
        {
            ...
            if (OSTN)
            {
                ngr = GPS2UKMapRef(deciLat, deciLon, height);
        ...
        public static (double latitude,double longitude)? ConvertmapReferenceToWGS4(string mapReference)
        {
            (double latitude, double longitude)? result = null;
            if (string.IsNullOrEmpty(mapReference)) return result;
            mapReference = mapReference.Trim().Replace(" ", "");
            string mapRefRegex = "[A-HJ-Z]{2}\\d{6}";
            string mapRefRegex4 = "[A-HJ-Z]{2}\\d{4}";
            if(Regex.Match(mapReference, mapRefRegex).Success || Regex.Match(mapReference,mapRefRegex4).Success)
            {
                var eastingnorthing = ConvertMapReference2EN(mapReference);
                if (eastingnorthing.easting > 0)
                {
                    Cartesian cartesian=GeoUK.Convert.ToCartesian(new Airy1830(),
                        new BritishNationalGrid(),
                        new EastingNorthing(eastingnorthing.easting,eastingnorthing.northing));
                    ...
                    result=new (wgsLatLong.Latitude, wgsLatLong.Longitude);
        ...
        public static (int easting,int northing) ConvertMapReference2EN(string mapRef)
        {
            (int easting, int northing) result = (-1, -1);
            if (string.IsNullOrEmpty(mapRef)) return ((-1, -1));
            string letters = mapRef.Substring(0, 2).ToUpper();
            string numeric=mapRef.Substring(2);

            

            string numericE = numeric.Substring(0,numeric.Length / 2);
            string numericN=numeric.Substring(numeric.Length / 2);

            while (numericE.Length < 5) numericE += "0";
            while(numericN.Length < 5)numericN += "0";

            

            int l1 = ((short)letters[0]) - (short)'A';
            int l2= ((short)letters[1])- (short)'A';
            if (l1 > 7) l1--;
            if (l2 > 7) l2--;

            // convert grid letters into 100km-square indexes from false origin (grid square SV):
            int e100km = ((l1 - 2) % 5) * 5 + (l2 % 5);
            int n100km = (19 - (int)(Math.Floor(l1 / 5.0d) * 5)) - (int)Math.Floor(l2 / 5.0d);

            numericE = $"{e100km}{numericE}";
            numericN = $"{n100km}{numericN}";

            if(int.TryParse(numericE, out int easting) && int.TryParse(numericN,out int northing))
            {
        ...
        public static string GPS2UKMapRef(double WGLat,double WGLong,double height)
        {
            LatitudeLongitude latlong=new LatitudeLongitude(WGLat,WGLong,height);
            Osgb36 bng = GeoUK.OSTN.Transform.Etrs89ToOsgb(latlong);
            return (bng.MapReference);
        }

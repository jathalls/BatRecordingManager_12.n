...
namespace UniversalToolkit
{
    ...
    public class ReferenceCall
    {
        ...
        /// <summary>
        /// adds this call to the currently saved settings in AppData\Roaming\Echolocation\WinBLP
        /// </summary>
        /// <returns></returns>
        public string setToSettings()
        {
            /*
             * is currently failing in virtual machine but not in local development machine.
             * hence the number of reports to Tools.InfoLog.
             * Fails with an object does not exist exception but unclear where
             * */
            string data = "";
            string err = "";
            try
            {
                ...
                settingsPath = Path.Combine(settingsPath, "customSettings.json");
                var settingsConfig = new SettingsConfiguration().WriteTo.JsonNet(settingsPath).ReadFrom.JsonNet(settingsPath);
                data = settingsConfig.ReadFrom.ToString();
                var SettingsMgr = settingsConfig.CreateSettingsMgr<ISettingsObjectMgr>();
                if (SettingsMgr == null)
                {
                    err=$"Unable to create a SettingsMgr {settingsConfig.ToString()}\n";
                    return (err + data+$"\n at '{settingsPath}'\n");
                }
                CustomSettings settings = new CustomSettings();
                try
            ...
            {
                return($"setToSetting: {ex.Message}\n{err}\n{data}");
            }
            return ("");
        }

...
namespace BatPassAnalysisFW
{
    ...
    public class bpaPass : INotifyPropertyChanged
    {
        ...
        public void CreatePass(decimal thresholdFactor, decimal spectrumFactor)
        {
            ...
            float leadInms = utSettings.AnalysisFW.envelopeLeadInMS;
            if (leadInms <= 0.0f)
            {
                leadInms = 0.2f;
                //CrossSettings.Current.Set<float>("EnvelopeLeadInMS", leadInms);
                utSettings.AnalysisFW.envelopeLeadInMS = leadInms;
                changed = true;
            }
            ...
            float leadOutms=utSettings.AnalysisFW.envelopeLeadOutMS;
            if (leadOutms <= 0.0f)
            {
                leadOutms = 1.0f;
                //CrossSettings.Current.Set<float>("EnvelopeLeadOutMS", 1.0f);
                utSettings.AnalysisFW.envelopeLeadOutMS = leadOutms;
                changed = true;
            }
            if(changed) UTSettings.setSettings(utSettings);
